...
class ShopOrderPayment(Base, PartitionedModel):
    """
    Represents a payment made towards a shop order.
    Links to the user making the payment, the order being paid for,
    and the shop the order is from.
    ...
    shop_id = Column(
        UUID(as_uuid=True), 
        nullable=False,
        comment="ID of the shop the order is from"
    )
    ...
    user = relationship(
        "User",
        backref=backref("shop_order_payments", lazy="dynamic"),
    ...
    shop = relationship(
        "Shop",
        backref=backref("order_payments", lazy="dynamic"),
    ...
    __table_args__ = (
        UniqueConstraint('transaction_reference', 'partition_key', name='uq_shop_order_payments_transaction_ref'),
        ForeignKeyConstraint(
            ['user_id', 'partition_key'],
            ['data_playground.users.id', 'data_playground.users.partition_key'],
            name='fk_shop_order_payment_user',
        ...
        ForeignKeyConstraint(
            ...
            name='fk_shop_order_payment_order',
        ...
        ForeignKeyConstraint(
            ...
            name='fk_shop_order_payment_shop',
        ...
        ForeignKeyConstraint(
            ...
            name='fk_shop_order_payment_method',
    ...
    async def complete_payment(self, db):
        ...
        await GlobalEvent.create_user_event(
            db,
            EventType.shop_order_payment_success,
            self.user_id,
    ...
    async def fail_payment(self, db, error_message=None):
        ...
        await GlobalEvent.create_user_event(
            db,
            EventType.shop_order_payment_failed,
            self.user_id,
            payment_id=str(self.id),
            amount=self.amount,
            order_id=str(self.order_id),
    ...
    async def refund_payment(self, db, amount=None, reason=None):
        ...
        await GlobalEvent.create_user_event(
            db,
            EventType.shop_order_payment_refunded,
            self.user_id,
            payment_id=str(self.id),
